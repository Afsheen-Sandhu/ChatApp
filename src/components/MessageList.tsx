...
export default function MessageList({ messages, statusById = {}, currentUser }: Props) {
    ...
    return (
        <div ref={containerRef} className="h-96 overflow-y-auto p-4 space-y-3 bg-stone-100/10 ">
			{messages.length === 0 ? (
				...
				messages.map((msg) => (
					<div key={msg.id} className={`flex ${msg.type === 'join' || msg.type === 'leave' ? 'justify-center' : ''}`}>
                        ...
                        {currentUser && msg.name === currentUser && statusById[msg.id] && (
                            <div className="mt-1 flex justify-end">
                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                    ...
                                    {statusById[msg.id] === 'delivered' ? '✓✓ Delivered' : '✓ Sent'}
                                </span>
                            </div>
                        )}
                            </div>
